...
function distanceSolver(positions: Array<number>, costFunction: (pos: number, target: number) => number) {
    ...
    var smallestSumOfDifferences: number = Number.POSITIVE_INFINITY;
    var positionOfSmallestSum: number = 0;
    /* This is a simple heuristic; we're assuming that the best position
     * is likely close to between the median and the mean.
     */
    for (var i = Math.min(mean, median) - 1; i <= Math.max(mean, median) + 1; i++) {
        var sumOfDifferences: number = 0;
        positions.forEach((position: number) => {
            sumOfDifferences += costFunction(position, i);
        })

        if (sumOfDifferences < smallestSumOfDifferences) {
            smallestSumOfDifferences = sumOfDifferences;
            positionOfSmallestSum = i;
        }
    }

    console.log(`Position ${positionOfSmallestSum} has total difference ${smallestSumOfDifferences}`);
}